...
import {MeshBuilder, Color3, StandardMaterial, DynamicTexture, Scene, Mesh, KeyboardEventTypes}  from '@babylonjs/core';
...
export class Map2D{
    ...
    constructor(name: string, scene: Scene, layers: TileLayer<OSM>[], view: View, resolution: {width: number, height: number}, size: number) {
        this.name = name;
        this.scene = scene;
        this.layers = layers;
        this.target = 'map2D';
    ...
    createScales() {
        ...
        let scaleLon = scaleLinear().domain([0, this.resolution.width]).range([-(width/2),width/2])
        let scaleLat = scaleLinear().domain([this.resolution.height, 0]).range([-(height/2),height/2])
