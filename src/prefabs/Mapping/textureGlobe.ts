...
import {MeshBuilder, Color3, StandardMaterial, DynamicTexture, Scene, Mesh, KeyboardEventTypes, Vector2, Vector3, AxesViewer}  from '@babylonjs/core';
...
export class textureGlobe{
    ...
    createTexture() {
        ...
        let textureGround = new DynamicTexture("mapTexture", {width: this.resolution.x, height: this.resolution.y}, scene); 
...
export function createTextureGlobe(
    ...
    scene: Scene
        ...
        const layers: TileLayer<any>[] = options.layers || [ new TileLayer({source: new OSM(), extent: [-180, -90, 180, 90]}),  new TileLayer({
            source: new TileDebug()
          })];
        const view: View = options.view || new View({
