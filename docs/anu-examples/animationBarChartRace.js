...
export function animationBarChartRace(engine) {
  ...
  let interval = 150; //Time between years in milliseconds
  nextTimestep();
  ...
  function nextTimestep() {
    ...
    //Update the axes for this year. For now there is no updateAxes function, so we need to destroy and create a new one on each update
    let prevAxes = axes;
    scene.onAfterRenderObservable.addOnce(() => prevAxes?.CoT.dispose());   //We use these observables for now to ensure an axes is always rendered on the screen
    
    //Create axes
    scene.onBeforeRenderObservable.addOnce(() => {
        axes = anu.createAxes('axes', scene, { 
          parent: chart,
          scale:
          {
            x: scalePop,
          ...
          labelFormat: { 
            x: (pop) => {
              return (pop / 1000000) + "M"; 
            },
            y: (rank) => {
              return data.find(d => d.rank === Number(rank)).country;
            }
          },
          background: false,
          grid: false
