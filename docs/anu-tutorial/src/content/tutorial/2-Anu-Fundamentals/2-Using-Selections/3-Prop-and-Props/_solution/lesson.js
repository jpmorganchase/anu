...
export default (scene) => {
    ...
    let data = [
      ...
      // Will loop through boxes.selected list each time a method is called
      // boxes.prop("position", (d) => new Vector3(d.goals, d.assists, d.points))
      //       .prop("rotation.x", (d) => d.assists)
      //       .prop("scaling.y", (d) => d.points);

      // Will loop through boxes.selected once
      boxes.props({
        "position": (d) => new Vector3(d.goals, d.assists, d.points),
        "rotation.x": (d) => d.assists,
        "scaling.y": (d) => d.points
      })
